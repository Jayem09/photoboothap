          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <h3 className="text-xl font-semibold text-white">Customize your photo strip</h3>
                
                {/* Frame Color Section */}
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <label className="block text-sm font-medium text-gray-300 mb-3">Frame colour</label>
                  <div className="grid grid-cols-4 gap-2">
                    {['White', 'Black', 'Pink', 'Green', 'Blue', 'Yellow', 'Purple', 'Maroon'].map((color) => (
                      <button
                        key={color}
                        className={`px-3 py-2 rounded-lg text-sm font-medium border transition-all ${
                          selectedFrame?.name === color
                            ? 'bg-white text-gray-900 border-white'
                            : 'bg-white/10 text-white border-white/30 hover:bg-white/20'
                        }`}
                        onClick={() => handleFrameSelect({ id: color.toLowerCase(), name: color, src: '', preview: '', category: 'classic' })}
                      >
                        {color}
                      </button>
                    ))}
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <input
                      type="color"
                      className="w-8 h-8 border border-white/30 rounded cursor-pointer"
                      onChange={(e) => {
                        const color = e.target.value;
                        handleFrameSelect({ 
                          id: 'custom', 
                          name: 'Custom', 
                          src: `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" fill="${color}"><rect width="100" height="100"/></svg>`, 
                          preview: '', 
                          category: 'classic' 
                        });
                      }}
                ...
                {/* Stickers Section */}
                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
                  <label className="block text-sm font-medium text-gray-300 mb-3">Stickers</label>
                  <div className="space-y-2">
                    <button className="w-full px-3 py-2 rounded-lg text-sm font-medium bg-white/10 text-white border border-white/30 hover:bg-white/20 transition-all">
                      No Stickers
                    </button>
                    <button className="w-full px-3 py-2 rounded-lg text-sm font-medium bg-white/10 text-white border border-white/30 hover:bg-white/20 transition-all">
                      Girlvpop
        ...
        {viewMode === 'gallery' && (
          <div className="space-y-8">
            <div className="text-center">
              ...
              <p className="text-gray-300 text-lg">View, download, and share your photos</p>
            </div>
